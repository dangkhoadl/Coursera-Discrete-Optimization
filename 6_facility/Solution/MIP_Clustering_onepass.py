...
class MIP_Clustering(object):
    ...
    def __solve_MIP(self, N:int, facilities:List[Facility], M:int, customers:List[Customer]):
        ...
        # Cant solve
        if model.status != 1:
            raise Exception(f'{self._test_case} solve MIP failed, facilities size = {N}, customers size = {M}')
    ...
    def solve(self,
        ...
        CLUSTER_SIZE = 300
        num_clusters = self._M // CLUSTER_SIZE + 1
        ...
        # Greedy: Prioritise Solving the big -> small clusters
        clusters.sort(key=lambda x:len(x), reverse=True)

        # Log cluster info
        logger.info(f"{self._test_case} : num_centroids = {len(centroids)}")
        ...
        for customers in clusters:
            objective, assignment = self.__solve_MIP(
                N=self._N, facilities=self._facs,
            ...
            for c, f in assignment.items():
                best_assign[c] = f
                self._facs[f].setup_cost = 0
                self._facs[f].capacity -= self._cuss[c].demand
